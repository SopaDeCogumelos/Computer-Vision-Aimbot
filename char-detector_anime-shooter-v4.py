        ...
        if debug_enabled:
            ...
            if move_vector:
                ...
                self.canvas.create_line(center_x, center_y, center_x + vx, center_y + vy, 
                                        fill='pink', width=5, arrow=tk.LAST)
        ...
        while True:
            try:
                ...
                try:
                    ...
                    final_target = None
                    if aim_enabled and all_detections and (time.time() - last_action_time) > ACTION_COOLDOWN_SECONDS:
                        ...
                        if current_max_distance == MAX_AIM_DISTANCE_FOCUSED:
                            potential_targets = [det for det in all_detections
                                                 if det['class_name'] in FOCUS_TRIGGER_CLASSES
                                                 and does_box_intersect_circle(det['box'], capture_center_point, MAX_AIM_DISTANCE_FOCUSED)]
                            additional_targets = [min(potential_targets, key=lambda p: PRIORIDADES_DE_ALVO.index(p['class_name']))] if potential_targets else []
